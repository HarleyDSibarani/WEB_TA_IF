...
class Mahasiswa extends CI_Controller {
    ...
    public function index(){
        ...
        $data = array(  'title'     => 'Data Mahasiswa',
                        'mahasiswa' => $mahasiswa,
                        'isi'       => 'admin/admin2_SekKorTA/mahasiswa/list'
                        );
    ...
    public function tambah(){
        ...
        if ($valid->run()===FALSE) {
			    $data = array('title'       => 'Tambah Data Mahasiswa',
                              'mahasiswa'   => $mahasiswa,
                              'dosen'       => $dosen,
                              'isi'         => 'admin/admin2_SekKorTA/mahasiswa/tambah'
                            );
			     $this->load->view('admin/layout/wrapper',$data);
        }else {
            $i    = $this->input;
            ...
            redirect(base_url('admin/admin2/mahasiswa'));
        }
    }
    ...
    public function edit($id_mhs){
        ...
        if ($valid->run()===FALSE) {
			    $data = array('title'      => 'Edit Data Mahasiswa'.$mahasiswa->nama,
                              'mahasiswa'  => $mahasiswa,
                              'dosen'      => $dosen,
                              'isi'        => 'admin/admin2_SekKorTA/mahasiswa/edit'
                            );
			     $this->load->view('admin/layout/wrapper',$data);
        }else {
            $i    = $this->input;
